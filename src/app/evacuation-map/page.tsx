...
type EditorMode = "idle" | "pin" | "route";
...
const legendItems = [
  { label: "Current Location", sample: "pin" },
...
function getRouteSegments(points: MapPoint[]) {
...
export default function EvacuationMapPage() {
  ...
  const editorLocation =
    ...
    null;
  const selectedRouteSegments = selectedLocation
    ? getRouteSegments(selectedLocation.routePoints)
    : [];
  const selectedExit =
  ...
  function handleMapEditorClick(event: MouseEvent<HTMLDivElement>) {
    ...
    if (editorMode === "pin") {
      ...
      setEditorMode("idle");
      setEditorStatus(`Pin updated for ${editorLocation.label}.`);
      return;
    }
        ...
        <div className="mt-8 overflow-hidden rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          >
            ...
            >
              ...
              <svg
                aria-hidden="true"
                className="pointer-events-none absolute inset-0 h-full w-full [--route-stroke:2px] md:[--route-stroke:3px]"
                  ...
                  <>
                    <path
                      ...
                      style={{
                        ...
                        strokeWidth: "calc(var(--route-stroke) + 4px)",
                      }}
                      vectorEffect="non-scaling-stroke"
                    />
                    {selectedRouteSegments.map((segment, index) => (
                      <path
              ...
              {mapData.emergencyExits.map((exit) => (
                <div
                  key={exit.id}
                  aria-hidden="true"
              ...
              {mapData.assemblyAreas.map((area) => (
                <div
                  key={area.id}
                  aria-hidden="true"
                ...
                return (
                  <button
                    ...
                    className={`absolute flex -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border-2 outline-none focus:ring-4 focus:ring-[#0F4C5C]/25 ${
                      isSelected
                        ? "h-8 w-8 border-white bg-[#0F4C5C] text-white shadow-lg ring-4 ring-[#ffdf20]/70 sm:h-9 sm:w-9"
              ...
              {selectedLocation && (
                <span
                  aria-hidden="true"
                  className="pointer-events-none absolute z-10 -translate-x-1/2 translate-y-3 rounded-full border border-white bg-[#0F766E] px-2 py-1 text-[10px] font-bold text-white shadow-sm"
                  style={{
              ...
              {selectedExit && (
                <span
                  ...
                  style={{ left: `${selectedExit.x}%`, top: `${selectedExit.y}%` }}
                >
                  Exit
                </span>
              )}

              {selectedAssemblyArea && (
                <span
                  aria-hidden="true"
                  ...
                  style={{
                    left: `${selectedAssemblyArea.x}%`,
                    top: `${selectedAssemblyArea.y}%`,
                  }}
                >
                  Assembly Area
                </span>
        ...
        {selectedLocation && (
          <div className="mt-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-[0.75fr_1.25fr]">
              ...
              <div className="rounded-2xl border border-slate-200 bg-[#F8FAFC] p-5 md:col-span-2">
                <h3 className="text-sm font-bold text-[#0F4C5C]">
...
function LegendSample({
  ...
  if (sample === "route") {
    return <span className="h-1 w-8 rounded-full bg-[#0F766E]" />;
  }
