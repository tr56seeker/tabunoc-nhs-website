    ...
    <main className="min-h-screen bg-[#f7f3ea] text-slate-900">
      ...
      <section className="mx-auto max-w-7xl px-3 py-7 sm:px-6 lg:px-8 lg:py-12">
        ...
        <div className="grid gap-7 lg:grid-cols-[minmax(0,1fr)_380px] lg:items-start">
          <div className="overflow-hidden rounded-2xl bg-white/85 p-3 shadow-xl ring-1 ring-slate-200/70 backdrop-blur sm:p-6">
          ...
          <aside aria-labelledby="upcoming-heading" className="rounded-2xl bg-white/90 p-5 shadow-xl ring-1 ring-slate-200/70 backdrop-blur sm:p-6 lg:sticky lg:top-28">
            ...
            <div className="max-h-[680px] space-y-3 overflow-y-auto pr-1">
              {loading ? <p className="rounded-xl bg-slate-50 p-6 text-sm font-semibold text-slate-500">Loading activities…</p> : panelEvents.length ? panelEvents.map((event) => <EventRow key={event.id} event={event} selected={selectedEvent?.id === event.id} onSelect={setSelectedEvent} />) : <p className="rounded-xl border border-dashed border-slate-300 p-7 text-center text-sm font-semibold leading-6 text-slate-500">No public school activities listed for this date.</p>}
            </div>
          </aside>
        </div>

        <div className="mt-9 flex gap-3 rounded-xl border border-slate-200 bg-white/85 p-5 text-sm leading-6 text-slate-600 shadow-sm sm:p-6"><span className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[#ffdf20] font-black text-[#24313E]" aria-hidden="true">i</span><p><strong className="text-[#24313E]">Public information notice.</strong> Calendar entries are provided for public information and may be updated based on DepEd issuances, school memoranda, weather conditions, class suspensions, or other official announcements.</p></div>
      </section>
