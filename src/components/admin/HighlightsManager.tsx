...
export default function HighlightsManager() {
  ...
  const loadHighlights = useCallback(async () => {
    setLoading(true);
    try {
      const response = await fetch("/api/admin/home-highlights", {
        cache: "no-store",
      });
      ...
      setDrafts(
        Object.fromEntries(
          next.map((highlight) => [highlight.id, makeDraft(highlight)])
        )
      );
    } catch (error) {
      setMessage({
  ...
  async function createHighlight(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || "The highlight could not be saved.");
      }
  ...
  async function updateHighlight(
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || "The highlight could not be updated.");
      }
      ...
      setMessage({
        ...
        text:
          error instanceof Error
            ? error.message
            : "The highlight could not be updated.",
      });
    } finally {
      setUpdatingId(null);
    }
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <aside className="rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm leading-6 text-slate-700">
        <strong className="text-amber-900">
          For authorized school personnel only.{" "}
        </strong>
        Upload only photos cleared for official school publication. Avoid
        close-up learner photos, learner records, LRN, grades, addresses,
        private documents, or sensitive information.
      </aside>
        ...
        <form onSubmit={createHighlight} className="mt-6 grid gap-5">
          <div className="grid gap-5 md:grid-cols-2">
            <label className={labelClass}>
              ...
              <input
                ...
                required
                className={`${fieldClass} file:mr-3 file:rounded-lg file:border-0 file:bg-slate-100 file:px-3 file:py-1.5 file:font-semibold`}
              />
            </label>
            <label className={labelClass}>
              Title
          ...
          <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
            ...
            <label className={labelClass}>
              Status
              <input
                value="Choose with a button below"
                disabled
          ...
          <label className={labelClass}>
            Alt text
            <span className="text-xs font-normal text-slate-500">
              Optional. Briefly describe what is visible; the title is used as a
              fallback.
            </span>
            <input name="altText" maxLength={300} className={fieldClass} />
          </label>

          <label className={labelClass}>
            Facebook Post URL
            <span className="text-xs font-normal text-slate-500">
              Optional. Add the related official Facebook post link if available.
            </span>
            <input
              name="facebookUrl"
              type="url"
              maxLength={500}
              placeholder="https://www.facebook.com/tabunocnatlhs/posts/..."
              className={fieldClass}
            />
          </label>
        ...
        <div className="flex items-end justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              {loading ? "Loading..." : `${highlights.length} total`}
            </p>
          </div>
        </div>
        ...
        <div className="mt-5 grid gap-5">
          {!loading && highlights.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-slate-300 bg-white/70 p-10 text-center text-sm text-slate-500">
              No highlights yet. Upload the first one above.
            </div>
          ) : (
                ...
                >
                  <div className="grid lg:grid-cols-[15rem_1fr]">
                    <img
                      ...
                      className="h-56 w-full bg-slate-100 object-cover lg:h-full"
                    />
                    <div className="p-5 sm:p-6">
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        <span
                          className={`rounded-full px-3 py-1 text-xs font-semibold capitalize ring-1 ring-inset ${statusStyles[highlight.status]}`}
                        >
                          {highlight.status}
                        </span>
                      ...
                      <div className="mt-5 grid gap-4">
                        <label className={labelClass}>
                          ...
                          <input
                            ...
                            maxLength={120}
                            onChange={(event) =>
                              changeDraft(highlight.id, "title", event.target.value)
                            }
                            className={fieldClass}
                          />
                        </label>
                        ...
                        <label className={labelClass}>
                          Facebook Post URL
                          <span className="text-xs font-normal text-slate-500">
                            Optional. Add the related official Facebook post link
                            if available.
                          </span>
                          <input
                            type="url"
                      ...
                      <div className="mt-5 flex flex-wrap gap-2">
                        <button
                          disabled={busy}
                          onClick={() => void updateHighlight(highlight)}
                        ...
                        {highlight.status !== "published" && (
                          <button
                            disabled={busy}
                            onClick={() =>
                              void updateHighlight(highlight, "published")
                            }
                        ...
                        {highlight.status === "published" && (
                          <button
                            disabled={busy}
                            onClick={() => void updateHighlight(highlight, "draft")}
                            className="rounded-xl bg-slate-700 px-4 py-2.5 text-sm font-semibold text-white hover:bg-slate-800 disabled:opacity-50"
                          >
                            Unpublish / Set Draft
                          </button>
                        )}
                        {highlight.status !== "archived" && (
                          <button
                            disabled={busy}
                            onClick={() =>
                              void updateHighlight(highlight, undefined, true)
                            }
                            className="rounded-xl border border-rose-200 px-4 py-2.5 text-sm font-semibold text-rose-700 hover:bg-rose-50 disabled:opacity-50"
                          >
                            Archive
                          </button>
                        )}
